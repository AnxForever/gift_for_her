...
import type { SelfiePhoto } from "@/lib/photo-manager"

export default function PublicSelfieGalleryPage() {
  ...
  useEffect(() => {
    const loadPhotos = () => {
      ...
      // Get user's selfie photos
      const userPhotos = JSON.parse(localStorage.getItem(`gallery-photos-${user.id}`) || "[]")
      const selfiePhotos = userPhotos.filter((p: any) => p.category === "selfie")

      setPhotos(selfiePhotos)
      setLoading(false)
    }
    ...
    return (
      <div className="min-h-screen romantic-gradient flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading selfie photos...</p>
        </div>
      </div>
    )
  }
    ...
    <PermissionsProvider galleryOwner={username}>
      <div className="min-h-screen">
        ...
        <main className="pt-16">
          <TimelineSelfieGallery initialPhotos={photos} />
        </main>
      </div>
    </PermissionsProvider>
  )
